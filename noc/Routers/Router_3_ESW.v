...
Definition westsend: UInternalFunction reg_t empty_ext_fn_t :=
  {{ fun westsend (value: bits_t 10) : unit_t =>
    write0(west_out, value)
  }}.
...
Definition westreceive: UInternalFunction reg_t empty_ext_fn_t :=
  {{ fun westreceive () : bits_t 10 =>
    read0(west_in)
  }}.
...
Definition _routepr_r (r_addr2: bits_t 4) (southreceive eastreceive westreceive southsend eastsend westsend: UInternalFunction reg_t empty_ext_fn_t) 
: uaction reg_t empty_ext_fn_t :=
  UBind "r_addr" (USugar (UConstBits r_addr2))
  {{
    let m0 := southreceive() in (*router input policy will be added here*)
...
Definition to_action rl :=
    match rl with
    | route0_r => _routepr_r Ob~0~1~0~1 southreceive eastreceive westreceive southsend eastsend westsend
    end.
...
Module RouterTests.
  ...
  Lemma check_west:
    run_action r_west (rules route0_r)
    (fun ctxt =>
      let bits_r0 := ctxt.[west_out] in
      Bits.to_nat bits_r0 = 834
    ).
  Proof.
    check.
  Defined.
