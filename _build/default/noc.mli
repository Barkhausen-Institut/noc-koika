...
type ('fn_name_t, 'action) internalFunction' = { int_name : 'fn_name_t;
                                                 int_body : 'action }
...
| InternalCall of 'var_t tsig * type0 * 'var_t tsig
   * ('fn_name_t, ('pos_t, 'var_t, 'fn_name_t, 'reg_t, 'ext_fn_t) action)
     internalFunction'
...
val struct_idx : type0 struct_sig' -> char list -> structIdx -> struct_index

val struct_idx_hd :
  char list -> type0 -> (char list * type0) list -> char list -> structIdx

val struct_idx_tl :
  char list -> char list -> type0 -> (char list * type0) list -> char list ->
  structIdx -> structIdx
...
val refine_sig_tau :
  var_t tsig -> type0 -> ('a1 -> type0) -> ('a2 -> externalSignature) ->
  ('a1, 'a2) action' -> ('a1, 'a2) action'
...
module Helpers :
 sig
  ...
  val widen_fin_clause_5 :
    (int -> int -> le_t -> t -> t) -> int -> int -> le_t -> t -> t -> t

  val widen_fin : int -> int -> le_t -> t -> t

  val le_t_inj_clause_2 :
    (int -> int -> le_t -> le_t) -> int -> int -> le_t -> le_t -> le_t
...
module Instances :
 sig
  ...
  val regt_elems_clause_2 :
    (int -> Setup.reg_t list) -> int -> Setup.reg_t list -> Setup.reg_t list
...
module Router :
 ...
 sig
  ...
  val r_send : int -> Setup.reg_t -> (Setup.reg_t, Setup.ext_fn_t) function0

  val r_receive :
    int -> Setup.reg_t -> (Setup.reg_t, Setup.ext_fn_t) function0

  val _routestart_r :
    int -> int -> (Setup.reg_t, Setup.ext_fn_t) function0 -> (Setup.reg_t,
    Setup.ext_fn_t) function0 -> (Setup.reg_t, Setup.ext_fn_t) function0 ->
    Setup.ext_fn_t -> Setup.ext_fn_t -> (Setup.reg_t, Setup.ext_fn_t) action0
  ...
  val _routecenter_r :
    int -> int -> (Setup.reg_t, Setup.ext_fn_t) function0 -> (Setup.reg_t,
    Setup.ext_fn_t) function0 -> (Setup.reg_t, Setup.ext_fn_t) function0 ->
    (Setup.reg_t, Setup.ext_fn_t) function0 -> (Setup.reg_t, Setup.ext_fn_t)
    function0 -> Setup.ext_fn_t -> Setup.ext_fn_t -> (Setup.reg_t,
  ...
  val routecenterfn :
    int -> int -> Setup.reg_t -> Setup.reg_t -> Setup.reg_t -> Setup.ext_fn_t
    -> Setup.ext_fn_t -> (Setup.reg_t, Setup.ext_fn_t) action0
 end
...
module Actions :
 ...
 sig
  module Routerfns :
   sig
    ...
    val _routestart_r :
      int -> int -> (Setup.reg_t, Setup.ext_fn_t) function0 -> (Setup.reg_t,
      Setup.ext_fn_t) function0 -> (Setup.reg_t, Setup.ext_fn_t) function0 ->
      Setup.ext_fn_t -> Setup.ext_fn_t -> (Setup.reg_t, Setup.ext_fn_t)
      action0
    ...
    val _routecenter_r :
      ...
      Setup.ext_fn_t) function0 -> Setup.ext_fn_t -> Setup.ext_fn_t ->
  ...
  val to_action :
    int -> Setup.rule_name_t -> int -> Helpers.le_t -> (Setup.reg_t,
    Setup.ext_fn_t) action0
  ...
  | Coq_to_action_graph_equation_7 of int * t * int * Helpers.le_t
     * to_action_graph

  val to_action_graph_rect :
    'a1 -> (int -> Helpers.le_t -> 'a1) -> (int -> 'a1) -> (int -> int ->
    Helpers.le_t -> 'a1) -> (int -> t -> to_action_graph -> 'a1 -> 'a1) ->
    (int -> t -> int -> Helpers.le_t -> to_action_graph -> 'a1 -> 'a1) -> int
    -> Setup.rule_name_t -> int -> Helpers.le_t -> (Setup.reg_t,
    Setup.ext_fn_t) action0 -> to_action_graph -> 'a1

  val to_action_graph_correct :
    int -> Setup.rule_name_t -> int -> Helpers.le_t -> to_action_graph

  val to_action_elim :
    'a1 -> (int -> Helpers.le_t -> 'a1) -> (int -> 'a1) -> (int -> int ->
    Helpers.le_t -> 'a1) -> (int -> t -> 'a1 -> 'a1) -> (int -> t -> int ->
    Helpers.le_t -> 'a1 -> 'a1) -> int -> Setup.rule_name_t -> int ->
    Helpers.le_t -> 'a1

  val coq_FunctionalElimination_to_action :
    __ -> (int -> Helpers.le_t -> __) -> (int -> __) -> (int -> int ->
    Helpers.le_t -> __) -> (int -> t -> __ -> __) -> (int -> t -> int ->
    Helpers.le_t -> __ -> __) -> int -> Setup.rule_name_t -> int ->
    Helpers.le_t -> __

  val coq_FunctionalInduction_to_action :
    (int -> Setup.rule_name_t -> int -> Helpers.le_t -> (Setup.reg_t,
    Setup.ext_fn_t) action0) functionalInduction

  val schedule_clause_4 :
    (int -> int -> Helpers.le_t -> (pos_t, Setup.rule_name_t) scheduler) ->
    int -> (pos_t, Setup.rule_name_t) scheduler -> (pos_t, Setup.rule_name_t)
    scheduler
  ...
  | Coq_schedule_graph_refinement_6 of int * int * Helpers.le_t
     * schedule_graph * schedule_clause_5_graph
  and schedule_clause_4_graph =
  | Coq_schedule_clause_4_graph_equation_1 of int
     * (pos_t, Setup.rule_name_t) scheduler
  and schedule_clause_5_graph =
  | Coq_schedule_clause_5_graph_equation_1 of int * int * Helpers.le_t
     * (pos_t, Setup.rule_name_t) scheduler

  val schedule_clause_5_graph_mut :
    'a1 -> (int -> Helpers.le_t -> 'a1) -> (int -> Helpers.le_t -> 'a1) ->
    (int -> schedule_graph -> 'a1 -> schedule_clause_4_graph -> 'a2 -> 'a1)
    -> (int -> int -> Helpers.le_t -> schedule_graph -> 'a1 ->
    schedule_clause_5_graph -> 'a3 -> 'a1) -> (int -> (pos_t,
    Setup.rule_name_t) scheduler -> 'a2) -> (int -> int -> Helpers.le_t ->
    (pos_t, Setup.rule_name_t) scheduler -> 'a3) -> int -> int ->
    Helpers.le_t -> (pos_t, Setup.rule_name_t) scheduler -> (pos_t,
    Setup.rule_name_t) scheduler -> schedule_clause_5_graph -> 'a3
  ...
  val schedule_clause_4_graph_mut :
    ...
    schedule_clause_5_graph -> 'a3 -> 'a1) -> (int -> (pos_t,
    Setup.rule_name_t) scheduler -> 'a2) -> (int -> int -> Helpers.le_t ->
    (pos_t, Setup.rule_name_t) scheduler -> 'a3) -> int -> (pos_t,
    Setup.rule_name_t) scheduler -> (pos_t, Setup.rule_name_t) scheduler ->
  ...
  val schedule_graph_mut :
    ...
    Setup.rule_name_t) scheduler -> 'a2) -> (int -> int -> Helpers.le_t ->
    (pos_t, Setup.rule_name_t) scheduler -> 'a3) -> int -> int ->
    Helpers.le_t -> (pos_t, Setup.rule_name_t) scheduler -> schedule_graph ->
    'a1
  ...
  val schedule_graph_correct : int -> int -> Helpers.le_t -> schedule_graph

  val schedule_elim :
    'a1 -> (int -> Helpers.le_t -> 'a1) -> (int -> Helpers.le_t -> 'a1) ->
    (int -> (pos_t, Setup.rule_name_t) scheduler -> __ -> 'a1 -> 'a1) -> (int
    -> int -> Helpers.le_t -> (pos_t, Setup.rule_name_t) scheduler -> __ ->
    'a1 -> 'a1) -> int -> int -> Helpers.le_t -> 'a1

  val coq_FunctionalElimination_schedule :
    __ -> (int -> Helpers.le_t -> __) -> (int -> Helpers.le_t -> __) -> (int
    -> (pos_t, Setup.rule_name_t) scheduler -> __ -> __ -> __) -> (int -> int
    -> Helpers.le_t -> (pos_t, Setup.rule_name_t) scheduler -> __ -> __ ->
    __) -> int -> int -> Helpers.le_t -> __
...
module FNoc :
 ...
 sig
  module Coq_d :
   sig
    module Routerfns :
     sig
      ...
      val _routecenter_r :
        ...
        (Setup.reg_t, Setup.ext_fn_t) function0 -> (Setup.reg_t,
        Setup.ext_fn_t) function0 -> (Setup.reg_t, Setup.ext_fn_t) function0
        -> (Setup.reg_t, Setup.ext_fn_t) function0 -> Setup.ext_fn_t ->
        Setup.ext_fn_t -> (Setup.reg_t, Setup.ext_fn_t) action0
    ...
    val to_action_graph_rect :
      'a1 -> (int -> Helpers.le_t -> 'a1) -> (int -> 'a1) -> (int -> int ->
      Helpers.le_t -> 'a1) -> (int -> t -> to_action_graph -> 'a1 -> 'a1) ->
      (int -> t -> int -> Helpers.le_t -> to_action_graph -> 'a1 -> 'a1) ->
      int -> Setup.rule_name_t -> int -> Helpers.le_t -> (Setup.reg_t,
      Setup.ext_fn_t) action0 -> to_action_graph -> 'a1
    ...
    val schedule_clause_4 :
      (int -> int -> Helpers.le_t -> (pos_t, Setup.rule_name_t) scheduler) ->
      int -> (pos_t, Setup.rule_name_t) scheduler -> (pos_t,
    ...
    val schedule_elim :
      'a1 -> (int -> Helpers.le_t -> 'a1) -> (int -> Helpers.le_t -> 'a1) ->
      (int -> (pos_t, Setup.rule_name_t) scheduler -> __ -> 'a1 -> 'a1) ->
      (int -> int -> Helpers.le_t -> (pos_t, Setup.rule_name_t) scheduler ->
      __ -> 'a1 -> 'a1) -> int -> int -> Helpers.le_t -> 'a1

    val coq_FunctionalElimination_schedule :
      __ -> (int -> Helpers.le_t -> __) -> (int -> Helpers.le_t -> __) ->
      (int -> (pos_t, Setup.rule_name_t) scheduler -> __ -> __ -> __) -> (int
      -> int -> Helpers.le_t -> (pos_t, Setup.rule_name_t) scheduler -> __ ->
      __ -> __) -> int -> int -> Helpers.le_t -> __
  ...
  val to_action : Setup.rule_name_t -> (Setup.reg_t, Setup.ext_fn_t) action0

  type to_action_graph =
  | Coq_to_action_graph_equation_1 of Setup.rule_name_t

  val to_action_graph_rect :
    (Setup.rule_name_t -> 'a1) -> Setup.rule_name_t -> (Setup.reg_t,
    Setup.ext_fn_t) action0 -> to_action_graph -> 'a1

  val to_action_graph_correct : Setup.rule_name_t -> to_action_graph

  val to_action_elim : (Setup.rule_name_t -> 'a1) -> Setup.rule_name_t -> 'a1
  ...
  val schedule_graph_rect :
    'a1 -> (pos_t, Setup.rule_name_t) scheduler -> schedule_graph -> 'a1
