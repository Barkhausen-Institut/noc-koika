...
Definition reg_t:=reg_t (regno' nocsize).
 ...
 uaction (reg_t regno) (ext_fn_t interfacesize) :=
    ...
    to_action (S nocsize') H (rule_ nocsize' idx) :=
        ...
        if Nat.eqb n_idx 0 then
                      routestartfn 0 
                            (reg_ (regno' nocsize) (nat_to_fin 0 _)) 
                            (reg_ (regno' nocsize) (nat_to_fin nocsize' _)) 
 ...
 *)
 ...
 Module Setup(ND:Number).
   ...
   Equations? to_action (nocsize:nat) (H: 2<=nocsize) : nat :=
      
     to_action 1 (le_S nocsize H1) with H1 := {} ; 
     to_action (S nocsize') H  := _.
 
  Equations foo (x : nat) (H: 2<=x) (y : nat) : nat :=
