 ...
 struct
  ...
  let rec log2_iter k p q r0 =
    (fun fO fS n -> if n=0 then fO () else fS (n-1))
      ...
      (fun k' ->
      (fun fO fS n -> if n=0 then fO () else fS (n-1))
        (fun _ -> log2_iter k' (Stdlib.Int.succ p) (Stdlib.Int.succ q) q)
...
let rec vect_map n0 f v =
  (fun fO fS n -> if n=0 then fO () else fS (n-1))
    ...
    (fun n1 -> vect_cons n1 (f (vect_hd n1 v)) (vect_map n1 f (vect_tl n1 v)))
    n0
...
let eqDec_vect_cons h h0 =
  { eq_dec = (fun t1 t2 ->
    ...
    let vtl0 = t1.vtl in
    let vhd1 = t2.vhd in
    let vtl1 = t2.vtl in if h.eq_dec vhd0 vhd1 then h0.eq_dec vtl0 vtl1 else false) }
...
let element_offset_right sig0 idx =
  Bits.rmul (sub sig0.array_len (Stdlib.Int.succ (index_to_nat sig0.array_len idx)))
    (element_sz sig0)
...
let cSig_of_Sig n0 sig0 =
  { argSigs = (vect_map n0 type_sz sig0.argSigs); retSig = (type_sz sig0.retSig) }

(** val sig_of_CSig : int -> int _Sig -> type0 _Sig **)

let sig_of_CSig n0 sig0 =
  { argSigs = (vect_map n0 (fun x -> Bits_t x) sig0.argSigs); retSig = (Bits_t
    sig0.retSig) }
...
let map_intf_body f fn =
  { int_name = fn.int_name; int_argspec = fn.int_argspec; int_retSig = fn.int_retSig;
  ...
  { eq_dec =
    (let rec iHtau t1 t2 =
       ...
       | Enum_t sig0 ->
         (match t2 with
          | Enum_t sig1 ->
            let { enum_name = enum_name0; enum_size = enum_size0; enum_bitsize =
              enum_bitsize0; enum_members = enum_members0; enum_bitpatterns =
              enum_bitpatterns0 } = sig0
            in
            let { enum_name = enum_name1; enum_size = enum_size1; enum_bitsize =
              enum_bitsize1; enum_members = enum_members1; enum_bitpatterns =
              enum_bitpatterns1 } = sig1
            in
            ...
            then let s0 = eqDec_nat.eq_dec enum_size0 enum_size1 in
                 ...
                 then let s1 = eqDec_nat.eq_dec enum_bitsize0 enum_bitsize1 in
                      if s1
                      then let s2 =
                             (eqDec_vect enum_size1 eqDec_string).eq_dec enum_members0
                               enum_members1
                           in
                           if s2
       ...
       | Struct_t sig0 ->
         (match t2 with
          | Struct_t sig1 ->
            let { struct_name = struct_name0; struct_fields = struct_fields0 } = sig0
            in
            let { struct_name = struct_name1; struct_fields = struct_fields1 } = sig1
            in
                   ...
                   | y :: l0 ->
                     (match x with
                      ...
                      | p :: l1 ->
                        if let (a, b) = y in
                           let (s0, t) = p in
                           if eqDec_string.eq_dec a s0 then iHtau b t else false
                        then f l0 l1
                        else false)
...
module PrimTyped =
 struct
  ...
  let coq_GetElementBits sig0 idx =
    Slice ((array_sz sig0), (element_offset_right sig0 idx), (element_sz sig0))

  (** val coq_SubstElementBits : type0 array_sig' -> array_index -> fbits2 **)

  let coq_SubstElementBits sig0 idx =
    SliceSubst ((array_sz sig0), (element_offset_right sig0 idx), (element_sz sig0))

  (** val coq_GetFieldBits : type0 struct_sig' -> struct_index -> fbits1 **)

  let coq_GetFieldBits sig0 idx =
    Slice ((struct_sz sig0), (field_offset_right sig0 idx), (field_sz sig0 idx))

  (** val coq_SubstFieldBits : type0 struct_sig' -> struct_index -> fbits2 **)

  let coq_SubstFieldBits sig0 idx =
    SliceSubst ((struct_sz sig0), (field_offset_right sig0 idx), (field_sz sig0 idx))
 end
...
module PrimTypeInference =
 struct
  ...
  let find_field sig0 f =
    opt_result (list_assoc eqDec_string f sig0.struct_fields) (Arg1, (UnboundField (f,
      sig0)))
  ...
  let check_index sig0 pos =
    opt_result (index_of_nat sig0.array_len pos) (Arg1, (OutOfBounds (pos, sig0)))
       ...
       | Success sz1 ->
         Success (PrimTyped.Bits1
           (match fn0 with
            ...
            | PrimUntyped.USExt width -> PrimTyped.SExt ((Obj.magic sz1), width)
            | PrimUntyped.UZExtL width -> PrimTyped.ZExtL ((Obj.magic sz1), width)
            | PrimUntyped.UZExtR width -> PrimTyped.ZExtR ((Obj.magic sz1), width)
            | PrimUntyped.URepeat times -> PrimTyped.Repeat ((Obj.magic sz1), times)
         ...
         (match assert_kind (Kind_struct None) Arg1 tau1 with
          | Success sig0 ->
            (match find_field (Obj.magic sig0) f with
             | Success idx -> Success (PrimTyped.Struct1 ((Obj.magic sig0), idx))
             | Failure f0 -> Failure f0)
          | Failure f0 -> Failure f0)
         ...
         (match assert_kind (Kind_array None) Arg1 tau1 with
          | Success sig0 ->
            (match check_index (Obj.magic sig0) pos with
             | Success idx -> Success (PrimTyped.Array1 ((Obj.magic sig0), idx))
             | Failure f -> Failure f)
          | Failure f -> Failure f)
          ...
          | Success sz2 ->
            Success (PrimTyped.Bits2
              (match fn0 with
               ...
               | PrimUntyped.ULsl -> PrimTyped.Lsl ((Obj.magic sz1), (Obj.magic sz2))
               | PrimUntyped.ULsr -> PrimTyped.Lsr ((Obj.magic sz1), (Obj.magic sz2))
               | PrimUntyped.UAsr -> PrimTyped.Asr ((Obj.magic sz1), (Obj.magic sz2))
               | PrimUntyped.UConcat ->
               ...
               | PrimUntyped.UMul -> PrimTyped.Mul ((Obj.magic sz1), (Obj.magic sz2))
      ...
      (match fn0 with
       | PrimUntyped.USubstField f ->
         (match assert_kind (Kind_struct None) Arg1 tau1 with
          | Success sig0 ->
            (match find_field (Obj.magic sig0) f with
             | Success idx -> Success (PrimTyped.Struct2 ((Obj.magic sig0), idx))
             | Failure f0 -> Failure f0)
          | Failure f0 -> Failure f0)
      ...
      (match fn0 with
       | PrimUntyped.USubstElement pos ->
         (match assert_kind (Kind_array None) Arg1 tau1 with
          | Success sig0 ->
            (match check_index (Obj.magic sig0) pos with
             | Success idx -> Success (PrimTyped.Array2 ((Obj.magic sig0), idx))
             | Failure f -> Failure f)
          | Failure f -> Failure f)
       | PrimUntyped.USubstElementBits (sig0, pos) ->
         (match check_index sig0 pos with
          | Success idx ->
            Success (PrimTyped.Bits2 (PrimTyped.coq_SubstElementBits sig0 idx))
          | Failure f -> Failure f))
 end
...
module CircuitSignatures =
 struct
        ...
        (match fn with
         | PrimTyped.DisplayUtf8 len ->
           Array_t { array_type = (Bits_t (Stdlib.Int.succ (Stdlib.Int.succ
             ...
             (Stdlib.Int.succ (Stdlib.Int.succ 0))))))))); array_len = len }
         | PrimTyped.DisplayValue (tau, _) -> tau) (Obj.magic __)); retSig = (Bits_t
      0) }

  (** val coq_CSigma1 : PrimTyped.fbits1 -> int _Sig **)

  let coq_CSigma1 = function
  | PrimTyped.Not sz0 -> { argSigs = (vect_cons 0 sz0 (Obj.magic __)); retSig = sz0 }
  | PrimTyped.SExt (sz0, width) ->
    { argSigs = (vect_cons 0 sz0 (Obj.magic __)); retSig = (Nat.max sz0 width) }
  ...
  | PrimTyped.And sz0 ->
    { argSigs = (vect_cons (Stdlib.Int.succ 0) sz0 (vect_cons 0 sz0 (Obj.magic __)));
      retSig = sz0 }
  | PrimTyped.Or sz0 ->
    { argSigs = (vect_cons (Stdlib.Int.succ 0) sz0 (vect_cons 0 sz0 (Obj.magic __)));
      retSig = sz0 }
  ...
  | PrimTyped.Lsl (bits_sz, shift_sz) ->
    { argSigs =
      (vect_cons (Stdlib.Int.succ 0) bits_sz (vect_cons 0 shift_sz (Obj.magic __)));
      retSig = bits_sz }
  ...
  | PrimTyped.Concat (sz1, sz2) ->
    { argSigs = (vect_cons (Stdlib.Int.succ 0) sz1 (vect_cons 0 sz2 (Obj.magic __)));
      retSig = (add sz2 sz1) }
  | PrimTyped.Sel sz0 ->
    { argSigs =
      (vect_cons (Stdlib.Int.succ 0) sz0
        (vect_cons 0 (Nat.log2_up sz0) (Obj.magic __))); retSig = (Stdlib.Int.succ 0) }
  | PrimTyped.SliceSubst (sz0, _, width) ->
    { argSigs =
      (vect_cons (Stdlib.Int.succ 0) sz0 (vect_cons 0 width (Obj.magic __))); retSig =
      sz0 }
...
module PrimSignatures =
 struct
  ...
  | PrimTyped.Conv (tau, fn0) ->
    (match fn0 with
     | PrimTyped.Pack ->
       { argSigs = (vect_cons 0 tau (Obj.magic __)); retSig = (Bits_t (type_sz tau)) }
     | PrimTyped.Unpack ->
       { argSigs = (vect_cons 0 (Bits_t (type_sz tau)) (Obj.magic __)); retSig = tau }
     | PrimTyped.Ignore ->
  ...
  | PrimTyped.Array1 (sig0, _) ->
    { argSigs = (vect_cons 0 (Array_t sig0) (Obj.magic __)); retSig = sig0.array_type }

  (** val coq_Sigma2 : PrimTyped.fn2 -> type0 _Sig **)

  let coq_Sigma2 = function
  | PrimTyped.Eq (tau, _) ->
    { argSigs = (vect_cons (Stdlib.Int.succ 0) tau (vect_cons 0 tau (Obj.magic __)));
  ...
  | PrimTyped.Struct2 (sig0, idx) ->
    { argSigs =
      (vect_cons (Stdlib.Int.succ 0) (Struct_t sig0)
        (vect_cons 0 (field_type sig0 idx) (Obj.magic __))); retSig = (Struct_t sig0) }
  | PrimTyped.Array2 (sig0, _) ->
    { argSigs =
      (vect_cons (Stdlib.Int.succ 0) (Array_t sig0)
        (vect_cons 0 sig0.array_type (Obj.magic __))); retSig = (Array_t sig0) }
 end
...
let bits_of_ascii c =
 ...
 (fun f c ->
  ...
  f (h 0) (h 1) (h 2) (h 3) (h 4) (h 5) (h 6) (h 7))
    ...
    vect_cons (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
      (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ 0))))))) b0
      (vect_cons (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
        (Stdlib.Int.succ (Stdlib.Int.succ 0)))))) b1
        (vect_cons (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
          (Stdlib.Int.succ 0))))) b2
          (vect_cons (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
            (Stdlib.Int.succ 0)))) b3
            (vect_cons (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ 0))) b4
              (vect_cons (Stdlib.Int.succ (Stdlib.Int.succ 0)) b5
                (vect_cons (Stdlib.Int.succ 0) b6 (vect_cons 0 b7 (Obj.magic __)))))))))
    c
...
let uinit tau =
  let zeroes = UConst ((Bits_t (type_sz tau)), (vect_const (type_sz tau) false)) in
...
let ustruct_init sig0 fields =
  ...
  let usubst = fun f x x0 -> UBinop ((PrimUntyped.UStruct2 (PrimUntyped.USubstField
    f)), x, x0)
  in
...
(** val uswitch :
    ...
    'a2, 'a3, 'a4, 'a5) uaction * ('a1, 'a2, 'a3, 'a4, 'a5) uaction) list -> ('a1,
    'a2, 'a3, 'a4, 'a5) uaction **)
...
let desugar_action' =
  let rec desugar_action'0 pos fR fSigma a =
    ...
    (match a with
     ...
     | UInternalCall (fn, args) -> UInternalCall ((map_intf_body d fn), (map d args))
  ...
  and desugar _ _ pos fR fSigma s =
    ...
    (match s with
     ...
     | UConstString s0 ->
       UConst ((Array_t { array_type = (Bits_t (Stdlib.Int.succ (Stdlib.Int.succ
         (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
         (Stdlib.Int.succ (Stdlib.Int.succ 0))))))))); array_len = (length0 s0) }),
         (array_of_bytes s0))
     | UConstEnum (sig0, name) ->
       (match vect_index sig0.enum_size eqDec_string name sig0.enum_members with
        | Some idx ->
          UConst ((Enum_t sig0), (vect_nth sig0.enum_size sig0.enum_bitpatterns idx))
        | None -> UError { epos = pos; emsg = (UnboundEnumMember (name, sig0)) })
     | UProgn aa -> uprogn (map d aa)
     | ULet (bindings, body) ->
       fold_right (fun pat acc -> let (var, a) = pat in UBind (var, (d a), acc))
         (d body) bindings
     | UWhen (cond, body) -> UIf ((d cond), (d body), (UFail (Bits_t 0)))
     | USwitch (var, default, branches) ->
       let branches0 =
         map (fun pat -> let (cond, body) = pat in ((d cond), (d body))) branches
       in
     ...
     | UArrayInit (tau, elements) ->
       ...
       snd
         (fold_left (fun pat a ->
           let (pos0, acc) = pat in ((Stdlib.Int.succ pos0), (usubst pos0 acc (d a))))
           elements (0, empty))
...
let lift_fn1_tc_result r0 sigma sig0 tau pos e r1 =
  result_map_failure (fun pat ->
    let (_, err) = pat in lift_basic_error_message r0 sigma pos sig0 tau e err) r1

(** val lift_fn2_tc_result :
    ('a4 -> type0) -> ('a5 -> externalSignature) -> 'a2 tsig -> type0 -> 'a2 tsig ->
    type0 -> 'a1 -> ('a1, 'a2, 'a3, 'a4, 'a5) action -> 'a1 -> ('a1, 'a2, 'a3, 'a4,
    'a5) action -> ('a6, fn_tc_error) result -> ('a6, ('a1, 'a2, 'a3) error) result **)
...
(** val cast_action' :
    ...
    -> ('a1, 'a3, 'a2, 'a4, 'a5) action -> ('a3, 'a2) error_message -> (('a1, 'a3,
    'a2, 'a4, 'a5) action, ('a1, 'a3, 'a2) error) result **)
...
(** val cast_action :
    ('a4 -> type0) -> ('a5 -> externalSignature) -> 'a1 -> 'a3 tsig -> type0 -> type0
    -> ('a1, 'a3, 'a2, 'a4, 'a5) action -> (('a1, 'a3, 'a2, 'a4, 'a5) action, ('a1,
    'a3, 'a2) error) result **)

let cast_action r0 sigma pos sig0 tau1 tau2 e =
  cast_action' r0 sigma pos sig0 tau1 tau2 e (BasicError (TypeMismatch (tau1, tau2)))
...
(** val assert_argtypes' :
    ('a4 -> type0) -> ('a5 -> externalSignature) -> 'a3 tsig -> 'a6 -> int -> 'a2 ->
    'a1 -> 'a3 tsig -> ('a1 * (type0, ('a1, 'a3, 'a2, 'a4, 'a5) action) sigT) list ->
...
let rec assert_argtypes' r0 sigma sig0 src nexpected fn_name pos args_desc args =
  ...
  | [] ->
    (match args with
     | [] -> Success CtxEmpty
     | _ :: _ ->
       Failure (mkerror pos (TooManyArguments (fn_name, nexpected, (length args))) src))
  | p :: fn_sig ->
    let (name1, tau1) = p in
    (match args with
     | [] ->
       Failure
         (mkerror pos (TooFewArguments (fn_name, nexpected, (length args_desc))) src)
     | p0 :: args0 ->
       let (pos1, arg1) = p0 in
       (match cast_action r0 sigma pos1 sig0 (projT1 arg1) tau1 (projT2 arg1) with
        | Success arg2 ->
          (match assert_argtypes' r0 sigma sig0 src nexpected fn_name pos fn_sig args0 with
           | Success ctx -> Success (CtxCons (fn_sig, (name1, tau1), arg2, ctx))
           | Failure f -> Failure f)
        | Failure f -> Failure f))

(** val assert_argtypes :
    ('a4 -> type0) -> ('a5 -> externalSignature) -> 'a3 tsig -> 'a6 -> 'a2 -> 'a1 ->
    'a3 tsig -> ('a1 * (type0, ('a1, 'a3, 'a2, 'a4, 'a5) action) sigT) list ->
    (('a3 * type0, ('a1, 'a3, 'a2, 'a4, 'a5) action) context, ('a1, 'a3, 'a2) error)
    result **)

let assert_argtypes r0 sigma sig0 src fn_name pos args_desc args =
  assert_argtypes' r0 sigma sig0 src (length args_desc) fn_name pos args_desc args
...
| UVar var ->
  (match opt_result (assoc var_t_eq_dec var sig0) (mkerror pos (UnboundVariable var) e) with
   | Success tau_m ->
  ...
  (match opt_result (assoc var_t_eq_dec var sig0) (mkerror pos (UnboundVariable var) e) with
   | Success tau_m ->
     (match type_action var_t_eq_dec r0 sigma pos sig0 ex with
      | Success ex' ->
        (match cast_action r0 sigma (actpos pos ex) sig0 (projT1 ex') (projT1 tau_m)
                 (projT2 ex') with
         | Success ex'0 ->
...
| UBind (v, ex, body) ->
  (match type_action var_t_eq_dec r0 sigma pos sig0 ex with
   | Success ex0 ->
     (match type_action var_t_eq_dec r0 sigma pos ((v, (projT1 ex0)) :: sig0) body with
      | Success body0 ->
        Success (ExistT ((projT1 body0), (Bind (sig0, (projT1 ex0), (projT1 body0), v,
          (projT2 ex0), (projT2 body0)))))
      | Failure f -> Failure f)
   | Failure f -> Failure f)
| UIf (cond, tbranch, fbranch) ->
  (match type_action var_t_eq_dec r0 sigma pos sig0 cond with
   | Success cond' ->
     (match cast_action r0 sigma (actpos pos cond) sig0 (projT1 cond') (Bits_t
              (Stdlib.Int.succ 0)) (projT2 cond') with
      | Success cond'0 ->
        (match type_action var_t_eq_dec r0 sigma pos sig0 tbranch with
         | Success tbranch' ->
           (match type_action var_t_eq_dec r0 sigma pos sig0 fbranch with
            | Success fbranch' ->
              (match cast_action r0 sigma (actpos pos fbranch) sig0 (projT1 fbranch')
                       (projT1 tbranch') (projT2 fbranch') with
               | Success fbranch'0 ->
                 Success (ExistT ((projT1 tbranch'), (If (sig0, (projT1 tbranch'),
                   cond'0, (projT2 tbranch'), fbranch'0))))
               | Failure f -> Failure f)
            | Failure f -> Failure f)
         | Failure f -> Failure f)
      | Failure f -> Failure f)
...
| UWrite (port0, idx, value) ->
  (match type_action var_t_eq_dec r0 sigma pos sig0 value with
   | Success value' ->
     (match cast_action r0 sigma (actpos pos value) sig0 (projT1 value') (r0 idx)
              (projT2 value') with
      | Success value'0 ->
...
| UUnop (fn, arg1) ->
  ...
  (match type_action var_t_eq_dec r0 sigma pos sig0 arg1 with
   | Success arg1' ->
     (match lift_fn1_tc_result r0 sigma sig0 (projT1 arg1') pos1 (projT2 arg1')
              (PrimTypeInference.tc1 fn (projT1 arg1')) with
      | Success fn0 ->
        (match cast_action r0 sigma pos1 sig0 (projT1 arg1')
                 (vect_hd 0 (PrimSignatures.coq_Sigma1 fn0).argSigs) (projT2 arg1') with
         | Success arg1'0 ->
           Success (ExistT ((PrimSignatures.coq_Sigma1 fn0).retSig, (Unop (sig0, fn0,
             arg1'0))))
         | Failure f -> Failure f)
      | Failure f -> Failure f)
   | Failure f -> Failure f)
  ...
  (match type_action var_t_eq_dec r0 sigma pos sig0 arg1 with
   | Success arg1' ->
     (match type_action var_t_eq_dec r0 sigma pos sig0 arg2 with
      | Success arg2' ->
        (match lift_fn2_tc_result r0 sigma sig0 (projT1 arg1') sig0 (projT1 arg2')
                 pos1 (projT2 arg1') pos2 (projT2 arg2')
              ...
              (match cast_action r0 sigma pos2 sig0 (projT1 arg2')
                       (vect_hd 0
                         (vect_tl (Stdlib.Int.succ 0)
                           (PrimSignatures.coq_Sigma2 fn0).argSigs)) (projT2 arg2') with
               | Success arg2'0 ->
                 Success (ExistT ((PrimSignatures.coq_Sigma2 fn0).retSig, (Binop
                   (sig0, fn0, arg1'0, arg2'0))))
               | Failure f -> Failure f)
            | Failure f -> Failure f)
         | Failure f -> Failure f)
      | Failure f -> Failure f)
  ...
  (match type_action var_t_eq_dec r0 sigma pos1 sig0 arg1 with
   | Success arg1' ->
     (match cast_action r0 sigma pos1 sig0 (projT1 arg1')
      ...
      | Success arg1'0 ->
        Success (ExistT ((sigma fn).retSig, (ExternalCall (sig0, fn, arg1'0))))
      | Failure f -> Failure f)
   | Failure f -> Failure f)
| UInternalCall (fn, args) ->
  (match result_list_map (type_action var_t_eq_dec r0 sigma pos sig0) args with
   | Success tc_args ->
     ...
     (match assert_argtypes r0 sigma sig0 e fn.int_name pos (rev fn.int_argspec)
              (rev tc_args_w_pos) with
      | Success args_ctx ->
        (match type_action var_t_eq_dec r0 sigma (actpos pos fn.int_body)
                 (rev fn.int_argspec) fn.int_body with
         | Success fn_body' ->
           (match cast_action r0 sigma (actpos pos fn.int_body) (rev fn.int_argspec)
                    (projT1 fn_body') fn.int_retSig (projT2 fn_body') with
            | Success fn_body'0 ->
              Success (ExistT (fn.int_retSig, (InternalCall (sig0, fn.int_retSig,
                fn.int_name, fn.int_argspec, args_ctx, fn_body'0))))
            | Failure f -> Failure f)
         | Failure f -> Failure f)
      | Failure f -> Failure f)
   | Failure f -> Failure f)
| UAPos (pos0, e0) ->
  (match type_action var_t_eq_dec r0 sigma pos0 sig0 e0 with
   | Success e1 ->
     Success (ExistT ((projT1 e1), (APos (sig0, (projT1 e1), pos0, (projT2 e1)))))
   | Failure f -> Failure f)
...
let tc_action var_t_eq_dec r0 sigma pos sig0 expected_tau e =
  ...
  | Success a -> cast_action r0 sigma pos sig0 (projT1 a) expected_tau (projT2 a)
  | Failure f -> Failure f
 ...
 struct
  ...
  let basic_flit =
    { struct_name =
      ...
      0))) :: ((('s'::('r'::('c'::[]))), (Bits_t (Stdlib.Int.succ (Stdlib.Int.succ
      (Stdlib.Int.succ (Stdlib.Int.succ
      0)))))) :: ((('t'::('r'::('g'::('_'::('y'::[]))))), (Bits_t (Stdlib.Int.succ
      (Stdlib.Int.succ 0)))) :: ((('t'::('r'::('g'::('_'::('x'::[]))))), (Bits_t
      (Stdlib.Int.succ (Stdlib.Int.succ 0)))) :: ((('d'::('a'::('t'::('a'::[])))),
      (Bits_t (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
      (Stdlib.Int.succ 0))))))) :: []))))) }
  ...
  let router_address =
    { struct_name =
      ...
      0)))) :: ((('x'::[]), (Bits_t (Stdlib.Int.succ (Stdlib.Int.succ 0)))) :: [])) }
 end
 ...
 struct
  ...
  let r_send reg_name =
    { int_name = ('r'::('_'::('s'::('e'::('n'::('d'::[])))))); int_argspec =
      ((prod_of_argsig { arg_name = ('v'::('a'::('l'::('u'::('e'::[]))))); arg_type =
         (Bits_t Types.sz) }) :: []); int_retSig = (Bits_t 0); int_body = (UWrite (P0,
      reg_name, (UVar ('v'::('a'::('l'::('u'::('e'::[])))))))) }
  ...
  let r_receive reg_name =
    { int_name = ('r'::('_'::('r'::('e'::('c'::('e'::('i'::('v'::('e'::[])))))))));
      int_argspec = []; int_retSig = (Bits_t Types.sz); int_body = (URead (P0,
      reg_name)) }
  ...
  let _routestart_r r_addr2 r0_send r0_receive =
    UBind (('r'::('_'::('a'::('d'::('d'::('r'::[])))))), (USugar (UConstBits
      ...
      (Bits.of_nat (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
        0)))) r_addr2)))), (UBind (('m'::('0'::[])), (USugar (UCallModule
      ((Obj.magic id), (Obj.magic __), (Obj.magic r0_receive), []))), (UBind
      (('m'::('s'::('g'::[]))), (UUnop ((PrimUntyped.UConv (PrimUntyped.UUnpack
      (Struct_t Types.basic_flit))), (UVar ('m'::('0'::[]))))), (UBind
      (('n'::('e'::('w'::('_'::('d'::('a'::('t'::('a'::[])))))))), (UUnop
      ((PrimUntyped.UStruct1 (PrimUntyped.UGetField ('n'::('e'::('w'::[]))))), (UVar
      ('m'::('s'::('g'::[])))))), (UBind (('s'::('r'::('c'::('_'::('p'::[]))))),
      (UUnop ((PrimUntyped.UStruct1 (PrimUntyped.UGetField ('s'::('r'::('c'::[]))))),
      (UVar ('m'::('s'::('g'::[])))))), (UIf ((UBinop ((PrimUntyped.UBits2
      ...
      ('m'::('s'::('g'::[])))))), (UBind (('t'::('r'::('g'::('_'::('y'::[]))))),
      (UUnop ((PrimUntyped.UStruct1 (PrimUntyped.UGetField
      ('t'::('r'::('g'::('_'::('y'::[]))))))), (UVar ('m'::('s'::('g'::[])))))),
      (UBind (('s'::('r'::('c'::('_'::('x'::[]))))), (UUnop ((PrimUntyped.UStruct1
      ...
      ('t'::('r'::('g'::('_'::('x'::[])))))), (UVar
      ('s'::('r'::('c'::('_'::('x'::[])))))))), (USugar (UCallModule ((Obj.magic id),
      (Obj.magic __), (Obj.magic r0_send), ((UUnop ((PrimUntyped.UConv
      PrimUntyped.UPack), (UBinop ((PrimUntyped.UStruct2 (PrimUntyped.USubstField
  ...
  let _routecenter_r r_addr2 r0_send r1_send r0_receive r1_receive =
    UBind (('r'::('_'::('a'::('d'::('d'::('r'::[])))))), (USugar (UConstBits
      ...
      (Bits.of_nat (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
        0)))) r_addr2)))), (UBind (('m'::('0'::[])), (USugar (UCallModule
      ((Obj.magic id), (Obj.magic __), (Obj.magic r0_receive), []))), (UBind
      (('m'::('1'::[])), (USugar (UCallModule ((Obj.magic id), (Obj.magic __),
      (Obj.magic r1_receive), []))), (UBind (('m'::('s'::('g'::[]))), (UUnop
      ((PrimUntyped.UConv (PrimUntyped.UUnpack (Struct_t Types.basic_flit))), (UVar
      ('m'::('0'::[]))))), (UBind
      (('n'::('e'::('w'::('_'::('d'::('a'::('t'::('a'::[])))))))), (UUnop
      ((PrimUntyped.UStruct1 (PrimUntyped.UGetField ('n'::('e'::('w'::[]))))), (UVar
      ('m'::('s'::('g'::[])))))), (UBind (('s'::('r'::('c'::('_'::('p'::[]))))),
      (UUnop ((PrimUntyped.UStruct1 (PrimUntyped.UGetField ('s'::('r'::('c'::[]))))),
      (UVar ('m'::('s'::('g'::[])))))), (USeq ((UIf ((UBinop ((PrimUntyped.UBits2
      PrimUntyped.UAnd), (UBinop ((PrimUntyped.UEq true), (UVar
      ('s'::('r'::('c'::('_'::('p'::[])))))), (UVar
      ('r'::('_'::('a'::('d'::('d'::('r'::[]))))))))), (UVar
      ('n'::('e'::('w'::('_'::('d'::('a'::('t'::('a'::[]))))))))))), (USeq ((USugar
      (UCallModule ((Obj.magic id), (Obj.magic __), (Obj.magic r0_send), ((UUnop
      ((PrimUntyped.UConv PrimUntyped.UPack), (UBinop ((PrimUntyped.UStruct2
      (PrimUntyped.USubstField ('n'::('e'::('w'::[]))))), (UVar
      ('m'::('s'::('g'::[])))), (USugar (UConstBits ((Stdlib.Int.succ 0),
      ...
      ('t'::('r'::('g'::('_'::('x'::[])))))), (UVar
      ('s'::('r'::('c'::('_'::('x'::[])))))))), (USugar (UCallModule ((Obj.magic id),
      (Obj.magic __), (Obj.magic r1_send), ((UUnop ((PrimUntyped.UConv
      PrimUntyped.UPack), (UBinop ((PrimUntyped.UStruct2 (PrimUntyped.USubstField
      ('s'::('r'::('c'::[]))))), (UVar ('m'::('s'::('g'::[])))), (UVar
      ('r'::('_'::('a'::('d'::('d'::('r'::[]))))))))))) :: [])))), (UIf ((UBinop
      ((PrimUntyped.UBits2 (PrimUntyped.UCompare (false, CLt))), (UVar
      ...
      (0, (Obj.magic __)))))))))))))))))), (USugar (UConstBits (0,
      (Obj.magic __)))))), (UBind (('m'::('s'::('g'::('1'::[])))), (UUnop
      ((PrimUntyped.UConv (PrimUntyped.UUnpack (Struct_t Types.basic_flit))), (UVar
      ('m'::('1'::[]))))), (UBind
      (('n'::('e'::('w'::('_'::('d'::('a'::('t'::('a'::[])))))))), (UUnop
      ((PrimUntyped.UStruct1 (PrimUntyped.UGetField ('n'::('e'::('w'::[]))))), (UVar
      ...
      ('m'::('s'::('g'::('1'::[]))))))), (UIf ((UBinop ((PrimUntyped.UBits2
      PrimUntyped.UAnd), (UBinop ((PrimUntyped.UEq true), (UVar
      ('s'::('r'::('c'::('_'::('p'::[])))))), (UVar
      ('r'::('_'::('a'::('d'::('d'::('r'::[]))))))))), (UVar
      ('n'::('e'::('w'::('_'::('d'::('a'::('t'::('a'::[]))))))))))), (USeq ((USugar
      (UCallModule ((Obj.magic id), (Obj.magic __), (Obj.magic r1_send), ((UUnop
      ((PrimUntyped.UConv PrimUntyped.UPack), (UBinop ((PrimUntyped.UStruct2
      (PrimUntyped.USubstField ('n'::('e'::('w'::[]))))), (UVar
      ('m'::('s'::('g'::('1'::[]))))), (USugar (UConstBits ((Stdlib.Int.succ 0),
      (vect_cons 0 false (Obj.magic __))))))))) :: [])))), (UBind
      (('t'::('r'::('g'::('_'::('x'::[]))))), (UUnop ((PrimUntyped.UStruct1
      (PrimUntyped.UGetField ('t'::('r'::('g'::('_'::('x'::[]))))))), (UVar
      ('m'::('s'::('g'::('1'::[]))))))), (UBind
  ...
  let _routeend_r r_addr2 r0_send r0_receive =
    UBind (('r'::('_'::('a'::('d'::('d'::('r'::[])))))), (USugar (UConstBits
      ...
      ((PrimUntyped.UBits2 (PrimUntyped.UCompare (false, CGt))), (UVar
      ('t'::('r'::('g'::('_'::('x'::[])))))), (UVar
      ('s'::('r'::('c'::('_'::('x'::[])))))))), (UFail (Bits_t 0)), (UIf ((UBinop
      ((PrimUntyped.UBits2 (PrimUntyped.UCompare (false, CLt))), (UVar
      ('t'::('r'::('g'::('_'::('x'::[])))))), (UVar
      ('s'::('r'::('c'::('_'::('x'::[])))))))), (USugar (UCallModule ((Obj.magic id),
      (Obj.magic __), (Obj.magic r0_send), ((UUnop ((PrimUntyped.UConv
...
module NOCImpl =
 struct
  ...
  let schedule =
    Cons (Coq_router_4, (Cons (Coq_router_3, (Cons (Coq_router_2, (Cons (Coq_router_1,
      Done)))))))
    ...
    extract_success
      (let desugared =
         desugar_action dummyPos_unit.dummy_pos (UBind
           ...
           (UCallModule (id, (Obj.magic __), (Obj.magic Routerfns.r_receive Coq_r1),
           []))), (UBind (('m'::('s'::('g'::[]))), (UUnop ((PrimUntyped.UConv
           (PrimUntyped.UUnpack (Struct_t Types.basic_flit))), (UVar
           ('m'::('0'::[]))))), (UBind
           (('n'::('e'::('w'::('_'::('d'::('a'::('t'::('a'::[])))))))), (UUnop
           ((PrimUntyped.UStruct1 (PrimUntyped.UGetField ('n'::('e'::('w'::[]))))),
           (UVar ('m'::('s'::('g'::[])))))), (UBind
           (('s'::('r'::('c'::('_'::('p'::[]))))), (UUnop ((PrimUntyped.UStruct1
           (PrimUntyped.UGetField ('s'::('r'::('c'::[]))))), (UVar
           ...
           (UUnop ((PrimUntyped.UStruct1 (PrimUntyped.UGetField
           ('t'::('r'::('g'::('_'::('y'::[]))))))), (UVar ('m'::('s'::('g'::[])))))),
           (UBind (('s'::('r'::('c'::('_'::('x'::[]))))), (UUnop
           ((PrimUntyped.UStruct1 (PrimUntyped.UGetField ('x'::[]))), (UUnop
           ((PrimUntyped.UConv (PrimUntyped.UUnpack (Struct_t Types.router_address))),
           (UVar ('r'::('_'::('a'::('d'::('d'::('r'::[]))))))))))), (UBind
           (('s'::('r'::('c'::('_'::('y'::[]))))), (UUnop ((PrimUntyped.UStruct1
           (PrimUntyped.UGetField ('y'::[]))), (UUnop ((PrimUntyped.UConv
           (PrimUntyped.UUnpack (Struct_t Types.router_address))), (UVar
           ('r'::('_'::('a'::('d'::('d'::('r'::[]))))))))))), (UIf ((UBinop
           ((PrimUntyped.UBits2 (PrimUntyped.UCompare (false, CGt))), (UVar
           ('t'::('r'::('g'::('_'::('x'::[])))))), (UVar
           ('s'::('r'::('c'::('_'::('x'::[])))))))), (USugar (UCallModule (id,
           (Obj.magic __), (Obj.magic Routerfns.r_send Coq_r1), ((UUnop
           ((PrimUntyped.UConv PrimUntyped.UPack), (UBinop ((PrimUntyped.UStruct2
           ...
           (Obj.magic __))))))))))))))))
       in
       tc_action eqDec_string coq_R empty_Sigma dummyPos_unit.dummy_pos [] (Bits_t 0)
         (Obj.magic desugared))
  | Coq_router_2 ->
    extract_success
      (let desugared =
         desugar_action dummyPos_unit.dummy_pos (UBind
           ...
           ((Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ 0)))),
           (Bits.of_nat (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
             (Stdlib.Int.succ 0)))) (Stdlib.Int.succ 0))))), (UBind (('m'::('0'::[])),
           (USugar (UCallModule (id, (Obj.magic __),
           (Obj.magic Routerfns.r_receive Coq_r1), []))), (UBind (('m'::('1'::[])),
           (USugar (UCallModule (id, (Obj.magic __),
           ...
           ('t'::('r'::('g'::('_'::('x'::[])))))), (UVar
           ('s'::('r'::('c'::('_'::('x'::[])))))))), (USugar (UCallModule (id,
           (Obj.magic __), (Obj.magic Routerfns.r_send Coq_r2), ((UUnop
           ((PrimUntyped.UConv PrimUntyped.UPack), (UBinop ((PrimUntyped.UStruct2
           (PrimUntyped.USubstField ('s'::('r'::('c'::[]))))), (UVar
           ('m'::('s'::('g'::[])))), (UVar
           ('r'::('_'::('a'::('d'::('d'::('r'::[]))))))))))) :: [])))), (UIf ((UBinop
    ...
    extract_success
      (let desugared =
         desugar_action dummyPos_unit.dummy_pos (UBind
           ...
           ((Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ 0)))),
           (Bits.of_nat (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
             (Stdlib.Int.succ 0)))) (Stdlib.Int.succ (Stdlib.Int.succ 0)))))), (UBind
           (('m'::('0'::[])), (USugar (UCallModule (id, (Obj.magic __),
           (Obj.magic Routerfns.r_receive Coq_r2), []))), (UBind (('m'::('1'::[])),
           (USugar (UCallModule (id, (Obj.magic __),
           ...
           ('t'::('r'::('g'::('_'::('x'::[])))))), (UVar
           ('s'::('r'::('c'::('_'::('x'::[])))))))), (USugar (UCallModule (id,
           (Obj.magic __), (Obj.magic Routerfns.r_send Coq_r3), ((UUnop
           ((PrimUntyped.UConv PrimUntyped.UPack), (UBinop ((PrimUntyped.UStruct2
           (PrimUntyped.USubstField ('s'::('r'::('c'::[]))))), (UVar
           ('m'::('s'::('g'::[])))), (UVar
           ('r'::('_'::('a'::('d'::('d'::('r'::[]))))))))))) :: [])))), (UIf ((UBinop
      ...
      (let desugared =
         desugar_action dummyPos_unit.dummy_pos (UBind
           ...
           (Bits.of_nat (Stdlib.Int.succ (Stdlib.Int.succ (Stdlib.Int.succ
             ...
             0))))))), (UBind (('m'::('0'::[])), (USugar (UCallModule (id,
           (Obj.magic __), (Obj.magic Routerfns.r_receive Coq_r3), []))), (UBind
           (('m'::('s'::('g'::[]))), (UUnop ((PrimUntyped.UConv (PrimUntyped.UUnpack
           (Struct_t Types.basic_flit))), (UVar ('m'::('0'::[]))))), (UBind
           (('n'::('e'::('w'::('_'::('d'::('a'::('t'::('a'::[])))))))), (UUnop
           ((PrimUntyped.UStruct1 (PrimUntyped.UGetField ('n'::('e'::('w'::[]))))),
  ...
  let package =
    { ip_koika = { koika_var_names = show_string; koika_fn_names = show_string;
      ...
      koika_reg_init = (Obj.magic r); koika_reg_finite = { finite_index = (fun h ->
      match Obj.magic h with
      | Coq_r1 -> 0
      ...
      koika_ext_fn_types = (Obj.magic empty_Sigma); koika_rules = (Obj.magic rules);
      koika_rule_external = (fun _ -> false); koika_rule_names = { show0 = (fun h ->
      match Obj.magic h with
      | Coq_router_1 -> 'r'::('o'::('u'::('t'::('e'::('r'::('_'::('1'::[])))))))
      | Coq_router_2 -> 'r'::('o'::('u'::('t'::('e'::('r'::('_'::('2'::[])))))))
      | Coq_router_3 -> 'r'::('o'::('u'::('t'::('e'::('r'::('_'::('3'::[])))))))
      | Coq_router_4 -> 'r'::('o'::('u'::('t'::('e'::('r'::('_'::('4'::[])))))))) };
      koika_scheduler = (Obj.magic schedule); koika_module_name =
      ('N'::('o'::('C'::[]))) }; ip_verilog = { vp_ext_fn_specs =
      (Obj.magic empty_ext_fn_props) }; ip_sim = { sp_ext_fn_specs =
      (Obj.magic empty_ext_fn_props); sp_prelude = None } }
