...
Definition r_receive (reg_name: reg_t) : UInternalFunction reg_t empty_ext_fn_t :=
  {{ fun r_receive () : bits_t sz =>
   ...
   Definition _routestart_r
      (r_addr2: nat)
      (r0_send r0_receive: UInternalFunction reg_t empty_ext_fn_t)
      : uaction reg_t empty_ext_fn_t :=
        {{
                let r_addr := #(Bits.of_nat 2 r_addr2) in
                 let m0 := r0_receive() in
                 let new_data := getbits(basic_flit,m0, new) in
                 let src_p := getbits(basic_flit,m0, src) in

                 (( if (src_p != r_addr && new_data) then
                      let dest := getbits(basic_flit, m0, dest) in
                      if dest > r_addr then
                        r0_send(substbits(basic_flit, m0, src, r_addr))  (* The pack changes the source content in the package*)
...
{{
    ...
    r0_send(substbits(basic_flit, m0, src, r_addr))    
         else
          pass                                      
                     (* To stop the packet from being processed again *)
  else
...
Definition _routecenter_r (r_addr2: nat) 
(r0_send r1_send r0_receive r1_receive: UInternalFunction reg_t empty_ext_fn_t) 
: uaction reg_t empty_ext_fn_t :=
  {{
  let r_addr := #(Bits.of_nat 2 r_addr2) in
  ...
  (( if (src_p != r_addr && new_data) then
      ...
      if dest > r_addr then
      r1_send(substbits(basic_flit, m0, src, r_addr))
      else if dest < r_addr then
      r0_send(substbits(basic_flit, m0, src, r_addr))
      else
  ...
  pass ));
  let new_data := getbits(basic_flit,m1, new) in
  let src_p := getbits(basic_flit,m1, src) in
  (( if (src_p != r_addr && new_data) then
      r1_send(substbits(basic_flit ,m1, new, Ob~0));
      let dest := getbits(basic_flit, m1, dest) in
      (*r_addr[|2`d0| :+ 2] if not using struct*)
      if dest > r_addr then
      r1_send(substbits(basic_flit, m1, src, r_addr))
      else if dest < r_addr then
      r0_send(substbits(basic_flit, m1, src, r_addr))
      else
      pass    (*Pass to tile from this block*)
  else
  pass ))
...
Definition to_action rl :=
  match rl with
  | route0_r => _routestart_r 0 (r_send r0) (r_receive r0)
  | route1_r => _routecenter_r 1 (r_send r0) (r_send r1) (r_receive r0) (r_receive r1)
  | route2_r => _routecenter_r 2 (r_send r1) (r_send r2) (r_receive r1) (r_receive r2)
  | route3_r => _routeend_r 3 (r_send r2) (r_receive r2)
  end.
