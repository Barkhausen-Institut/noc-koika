...
Definition r_send (reg_name: reg_t) : function R empty_Sigma :=
  <{ fun r_send (value: bits_t sz) : unit_t =>
       write0(reg_name, value)
  }>.
Definition r_receive (reg_name: reg_t) : function R empty_Sigma :=
...
(r_addr2: nat)
(r0_send: function R empty_Sigma (tau:=unit_t) (sig:=[("value", bits_t sz)]))  
(r0_receive: function R empty_Sigma (tau:=bits_t sz))
: action R empty_Sigma :=
<{
    let r_addr := #(Bits.of_nat 4 r_addr2) in 
    ...
    (( if (src_p != r_addr && new_data) then 
        let trg_x := get@basic_flit(msg, trg_x) in
        let trg_y := get@basic_flit(msg, trg_y) in
        let src_x := get@router_address(unpack(struct_t router_address, r_addr), x) in
        (*r_addr[|2`d0| :+ 2] if not using struct*)
        if trg_x > src_x then
        r0_send(pack(subst@basic_flit(msg, src, r_addr)))
...
: action R empty_Sigma :=
  ...
  (( if (src_p != r_addr && new_data) then 
      ...
      let trg_y := get@basic_flit(msg, trg_y) in
      let src_x := get@router_address(unpack(struct_t router_address, r_addr), x) in
      let src_y := get@router_address(unpack(struct_t router_address, r_addr), y) in
      (*r_addr[|2`d0| :+ 2] if not using struct*)
      if trg_x > src_x then
      fail
      else if trg_x < src_x then
  ...
  (r_addr2: nat)
  (r0_send: function R empty_Sigma (tau:=unit_t) (sig:=[("value", bits_t sz)]))
  (r1_send: function R empty_Sigma (tau:=unit_t) (sig:=[("value", bits_t sz)]))
  (r0_receive: function R empty_Sigma (tau:=bits_t sz) (sig:=[]))
  (r1_receive: function R empty_Sigma (tau:=bits_t sz) (sig:=[]))
  : action R empty_Sigma := <{
let r_addr := #(Bits.of_nat 4 r_addr2) in 
let m0 := r0_receive() in (*router input policy will be added here*)
...
let trg_x := get@basic_flit(msg, trg_x) in
let trg_y := get@basic_flit(msg, trg_y) in
let src_x := get@basic_flit(unpack(struct_t router_address, r_addr), x) in
let src_y := get@basic_flit(unpack(struct_t router_address, r_addr), y) in
(*r_addr[|2`d0| :+ 2] if not using struct*)
if trg_x > src_x then
...
let new_data := get@basic_flit(msg1, new) in
let src_p := get@basic_flit(msg1, src) in
(( if (src_p != r_addr && new_data) then
r1_send(substbits(basic_flit ,m1, new, Ob~0));
let trg_x := get@basic_flit(msg1, trg_x) in
let trg_y := get@basic_flit(msg1, trg_y) in
let src_x := get@basic_flit(unpack(struct_t router_address, r_addr), x) in
let src_y := get@basic_flit(unpack(struct_t router_address, r_addr), y) in
(*r_addr[|2`d0| :+ 2] if not using struct*)
if trg_x > src_x then
